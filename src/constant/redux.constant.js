//USER ACTIONS

export const SET_USER = 'SET_USER';
export const LOGOUT_USER = 'LOGOUT_USER';
export const SET_AUTH_TOKEN = 'SET_AUTH_TOKEN';
export const CLEAR_USER = 'CLEAR_USER';
export const REFRESH_USER = 'REFRESH_USER';

export const SWITCH_MODE = 'SWITCH_MODE';

export const FETCH_GAMES = 'FETCH_GAMES';
export const SET_GAMES = 'SET_GAMES';
export const SET_GAMES_LOADING = 'SET_GAMES_LOADING';

export const FETCH_TOURNAMENTS = 'FETCH_TOURNAMENTS';
export const SET_TOURNAMENTS = 'SET_TOURNAMENTS';
export const SET_TOURNAMENTS_LOADING = 'SET_TOURNAMENTS_LOADING';

export const SET_ORGANIZER = 'SET_ORGANIZER';

export const FETCH_BATTLE = 'FETCH_BATTLE';
export const SET_BATTLE_LOADING = 'SET_BATTLE_LOADING';
export const SET_BATTLE = 'SET_BATTLE';

export const CHECK_UPDATE = 'CHECK_UPDATE';

export const FETCH_ALL_JOINED_MATCH = 'FETCH_ALL_JOINED_MATCH';
export const SET_ALL_JOINED_MATCH = 'SET_ALL_JOINED_MATCH';
export const SET_ALL_JOINED_MATCH_LOADING = 'SET_ALL_JOINED_MATCH_LOADING';