...
import { GetTournamentQuery } from '../../graphql/graphql.query';
import { PublishTournamentMutation, DeleteTournamentMutation } from '../../graphql/graphql-mutation';

function ManageTournamentScene({ navigation }) {
    const params = AccessNestedObject(navigation, 'state.params', {});
    const id = AccessNestedObject(params, 'id', {});
    const [tournament, setTournament] = useState();

    const { loading } = useQuery(GetTournamentQuery, {
        variables: {
            id
        },
    ...
    const [publishTournament] = useMutation(PublishTournamentMutation, {
        variables: {
            id
        },
        onCompleted({ publishTournament }) {
    ...
    const endTime = AccessNestedObject(tournament, 'tournament_end');
    const tournamentMeta = AccessNestedObject(tournament, 'tournament_meta');
    const tournamentPayment = AccessNestedObject(tournament, 'tournament_payment');
    const tournamentRoom = AccessNestedObject(tournament, 'tournament_room');
    ...
    function handleNavigation(route) {
        ...
        const tournamentId = AccessNestedObject(tournament, 'id');

        switch (route) {
            case "basic_details":
                if (!isBasicDetailsSet())
    ...
    function isBasicDetailsSet() {
        return (Array.isArray(tournamentMeta) && tournamentMeta.length)
    }

    function isPrizeDetailSet() {
        return !!tournamentPayment;
    }

    function isTournamentRoomSet() {
    ...
    function RenderTournamentSetDetail() {
        return (
            <View style={{ flex: 1 }} >
                <MenuItem
                    iconName="gamepad"
                    ...
                    onPress={() => handleNavigation("basic_details")}
                    complete={isBasicDetailsSet()}
                />
                <MenuItem
                    ...
                    onPress={() => handleNavigation("prize_meta")}
                    complete={isPrizeDetailSet()}
                />
            </View>
        )
    }
        ...
        return (
            <View style={{ flex: 1, paddingBottom: 100 }} >
                <MenuItem
                    ...
                    onPress={() => handleNavigation("room_detail")}
                    complete={isTournamentRoomSet()}
                    inactive={!isPending()}
                />
                <MenuItem
                    iconName="comment"
                    ...
                    <>
                        <View style={styles.detailContainer} >
                            <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }} >
                                <Image source={{ uri: AccessNestedObject(tournament, 'game.thumbnail') }} style={styles.image} />
                            </View>
                            <View style={{ flex: 2, alignItems: 'flex-start', marginLeft: 20 }} >
                                <View style={styles.padding} >
                                    <Text style={styles.bold} numberOfLines={2} >
                                        {AccessNestedObject(tournament, 'name')}
                                    </Text>
                                </View>
                                ...
                                <View style={[styles.padding, { flexDirection: 'row', alignItems: 'center' }]} >
                                    ...
                                    <Text style={[styles.normal, { marginRight: 10, fontSize: 10 }]} >
                                        {GetReadableDate(tournament?.registration_start)}
                                    </Text>
                                </View>
                                <View style={[styles.padding, { flexDirection: 'row', alignItems: 'center' }]} >
                                    <Text style={[styles.light, { marginRight: 10 }]} >
                                    ...
                                    <Text style={[styles.normal, { marginRight: 10, fontSize: 10 }]} >
                                        {GetReadableDate(tournament?.registration_end)}
                                    </Text>
                                </View>
                                <View style={[styles.padding, { flexDirection: 'row', alignItems: 'center' }]} >
                                    <Text style={[styles.light, { marginRight: 10 }]} >
                                    ...
                                    <Text style={[styles.normal, { marginRight: 10, fontSize: 10 }]} >
                                        {GetReadableDate(tournament?.tournament_start)}
                                    </Text>
                                </View>
                                <View style={[styles.padding, { flexDirection: 'row', alignItems: 'center' }]} >
                                    <Text style={[styles.light, { marginRight: 10 }]} >
                                ...
                                {
                                    status == "active" && !moment().isAfter(moment.unix(endTime)) ?
                                        <Button
                                            onPress={finish}
                        ...
                        <Tabs
                            initialPage={getInitialIndex()}
                        >
                            <RenderTournamentSetDetail tabLabel="Set Detail" />
                            <RenderTournamentManage tabLabel="Manage" />
                            <RenderTournamentPost tabLabel="Post Tournament" />
                        </Tabs>
                    </> : null
            }
            <Spinner
