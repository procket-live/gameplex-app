...
function RenderHeader({ tournament, participents }) {
    ...
    participents.forEach((item) => {
        totalCollection += parseInt(AccessNestedObject(item, 'wallet_transaction.amount'));
    })
        ...
        <View style={{ borderBottomColor: GREY_BG, borderBottomWidth: 1, }} >
            <View style={{ height: 50, flexDirection: 'row' }} >
                ...
                <View style={{ flex: 1, alignItems: 'flex-start', justifyContent: 'center' }} >
                    <Text style={{ color: GREEN, fontSize: 18 }} >+ {DisplayPrice(AccessNestedObject(item, 'wallet_transaction.amount'))}</Text>
                </View>
            </View>
