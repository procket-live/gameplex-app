...
class PrivateApi {
    ...
    static AddWithdrawRequest = (amount) => {
        return Post({ url: WITHDRAW, body: { amount } })
    }

    static GetWithdrawRequest = (status = "pending") => {
        return Get({ url: `${WITHDRAW}/${status}` });
    }

    static UpdateParticipentScore = (id, score) => {
        return Post({ url: UPDATE_SCORE, body: { id, score } });
    }
}
